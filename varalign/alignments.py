...
def index_seq_to_alignment_FRAGSYS_CUSTOM(sequence, query_pdb_resnums, gap_chars=set(['-', '.']),
                           zero_index=False, reverse_mapping=False):
    """
    Index the residue numbers of a sequence to its alignment.
    ...
    alignment_index = [i for i, x in enumerate(seq_characters, start=index_start) if x not in gap_chars]
    start, end = get_start_end(sequence)
    sequence_index = query_pdb_resnums
    assert len(alignment_index) == len(sequence_index)
    if reverse_mapping:
        return list(zip(sequence_index, alignment_index))
    else:
...
def alignment_info_table_FRAGSYS_CUSTOM(alignment, mapping_dict, id_filter=''):
    """
    Build a table with key alignment info.
    ...
    for sequence in alignment:
        if 'query' in sequence.id:
            resnums = list(mapping_dict.values())
            first_resnum = resnums[0]
            last_resnum = resnums[-1]
            alignment_info.append((sequence.id,
                                   sequence.name,
                                   get_accession(sequence),
                                   (first_resnum, last_resnum),
                                   index_seq_to_alignment_FRAGSYS_CUSTOM(sequence, resnums)))
        elif id_filter in sequence.id:
            alignment_info.append((sequence.id,
                                   sequence.name,
                                   get_accession(sequence),
                                   get_start_end(sequence),
