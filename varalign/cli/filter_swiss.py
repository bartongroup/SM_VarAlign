...
sp_whitelist = os.path.join(
    os.path.dirname(varalign.__file__), "data", "swissprot_uniprot-all.tab.gz"
)
...
def main():
    ...
    parser.add_argument(
        "--whitelist",
        type=str,
        help="Path to sequence whitelist (e.g. SwissProt",
        default=sp_whitelist,
    )
    args = parser.parse_args()

    alignment = AlignIO.read(args.alignment, format="stockholm")
    new_alignment = pfam.filter_non_swissprot(
        alignment, swissprot_id_file=args.whitelist
    )
