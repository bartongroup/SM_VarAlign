...
def get_row_residue_numbers(subseq, uniprot_seq, use_local_alignment):
    ...
    if use_local_alignment:
        ...
        log.debug(
            "Aligning sub-sequence: {} to UniProt sequence: {}".format(
                subseq.id, uniprot_seq.id
            )
        )
        ...
        res_nums = [
            i + 1 for i, s in enumerate(seq) if s != "-"
        ]  # TODO: wrong if other seq has gaps too
        ...
        if str_seq in uniprot_str_seq:
            log.debug(
                "Matching sub-sequence: {} to UniProt sequence: {}".format(
                    subseq.id, uniprot_seq.id
                )
            )
            ...
            log.error(
                "Sub-sequence: {} does not match UniProt sequence: {}".format(
                    subseq.id, uniprot_seq.id
                )
            )
            log.error(
                "Sub-sequence: {} does not match UniProt sequence: {}".format(
                    str_seq, uniprot_str_seq
                )
            )
            raise TypeError
...
def map_columns_to_res_nums(col_nums, aligned_res_nums):
    ...
    record = {
        "seq_id": seq_id,
        "uniprot_seq_id": uniprot_seq_id,
        "uniprot_res_num": res_nums,
