...
def _structure_column_counts(
    ...
    gr_a = (
        sub_table.groupby(["SOURCE_ID_A", "Alignment_column_A"])
        .size()
    ...
    try:
        gr_b = (
            sub_table.groupby(["SOURCE_ID_B", "Alignment_column_B"])
            .size()
            .groupby("Alignment_column_B")
        )
...
def _column_ligand_contacts(aligned_prointvar_table):
    ...
    :return:
    """
    return _structure_column_counts(
        aligned_prointvar_table,
        query='interaction_type == "Protein-Ligand"',
...
def _column_set_feature_enrichment(
    ...
    test_df = column_annotation_table.groupby(groups).sum()[
...
def collect_column_structure_stats(aligned_prointvar_table):
    ...
    structure_stats = _interpolate_index(
        structure_stats
    )  # TODO: index to *full* alignment?
...
def column_set_ppi_enrichment(column_annotation_table, selection_query):
    ...
    :return:
    """
    return _column_set_feature_enrichment(
        column_annotation_table,
        selection_query,
