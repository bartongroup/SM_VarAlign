...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(
        description="Create variant counts column wise for an family."
    )
    ...
    parser.add_argument(
        "--jabaws_jar",
        type=str,
        default="/Applications/min-jabaws-client-2.1.0.jar",
        help="Path to JABAWS client.",
    )
    parser.add_argument(
        "--jabaws_server",
        type=str,
        default="http://www.compbio.dundee.ac.uk/jabaws",
    ...
    family_name = family.annotations["GF"]["AC"][
        0
    ]  # This requires biopython patches #768 #769
    ...
    for column in range(family.get_alignment_length()):
        ...
        contains_unk_chars.append(
            any([x not in ALIGNMENT_CHARS for x in family[:, column]])
        )
    log.info(
    ...
    cons_scores = pd.read_table(
        scores_out, sep=" ", comment=">", index_col=0, header=None
    )
