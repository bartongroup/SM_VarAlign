...
def _reformat_alignment_for_aacon(aln):
    ...
    aacon_alignment = apply_column_mask(
        aacon_alignment, column_mask
    )  # TODO: substitute illegal characters instead of masking those columns
...
def _parse_aacon_results(aacon_output, column_index=None):
    ...
    aacon_table = pd.read_table(
        aacon_output, sep=" ", comment=">", index_col=0, header=None
    )
    aacon_table = (
        aacon_table.transpose().dropna()
    )  # Needed to add `dropna` as extra row...
