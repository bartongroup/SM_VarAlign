...
def run_fisher_tests(alignment, table_mask, merged_table):
    ...
    non_variant_columns = [
        ...
        if i not in t["alignment_col_num"].unique()
    ]

    # Count gaps per column
    gaps_per_column = [
        alignment[:, i].count("-") for i in range(alignment_length)
    ]  # slow

    # Count variants at each column
    cross_table = pd.crosstab(
        t["seq_id"], t["alignment_col_num"]
    )  # TODO: Only has non-variant residues if protein has variant elsewhere
    ...
    for col_num in range(alignment_length):
        ...
        if col_num in cross_table.columns:
            ...
            non_variant_other = (
                sum((cross_table.drop(col_num, axis=1) == 0).sum())
        ...
        odds_ratio, pvalue = fisher_exact(
            [
                [variants_in_column, variants_in_other],
...
def _non_variant_sequences(alignment, variant_table):
    ...
    non_variant_sequences = [
        a for a in all_sequence_ids if a not in sequences_with_variants
    ]
