...
class TestVCFReader(TestCase):
    ...
    def test_clinvar_init(self):
        ...
        try:
            gnomad.Reader(filename=os.path.join(self.data_dir, 'sample_clinvar_vep.vcf.gz'))
    ...
    def test_clinvar_fetch(self):
        ...
        parser = gnomad.Reader(filename=os.path.join(self.data_dir, 'sample_clinvar_vep.vcf.gz'))
        try:
    ...
    def test_default_encoding(self):
        """Ensure explicitly specified Reader.encoding is not overridden."""
        parser = gnomad.Reader(filename=os.path.join(self.data_dir, 'sample_clinvar_vep.vcf.gz'))
