...
class TestProintvarAnalysis(TestCase):
    ...
    def setUpClass(cls):
        ...
        shutil.copytree(
            prointvar_db_path, os.path.join(test_dir, ".prointvar")
        )  # Copy .prointvar
        os.chdir(test_dir)
    ...
    def setUp(self):
        ...
        # Expected output
        output_files = [
            ".structural.figures.pdf",
            "_column_data.csv",
            "_seq_structure_mappings.csv",
        ]
        output_files = [
            "sample_swissprot_PF00001.18_full.sto" + s for s in output_files
        ]
    ...
    def test_tabular_output_is_stable(self):
        ...
        for f, standard_df, test_df in zip(
            csv_output, csv_tables_standard, csv_tables_test
        ):
        ...
        message = "The following table(s) / column(s) do not match their standards: {}".format(
            mismatched_tables
        )
